...
function App() {
  ...
  const applyParsedData = (data) => {
    ...
    const autoDetectedMapping = autoDetectColumns(newColumns)
  ...
  const handleClearData = () => {
    setParsedData([])
    setColumns([])
    setColumnMappings({ levels: [''], midColumn: undefined, levelAliases: {} })
    setActiveMappings({ levels: [], midColumn: undefined, levelAliases: {} })
    setCompanyName('')
    setIsSampleData(false)
    setError('')
    ...
    <div className="min-h-screen bg-gray-50 font-inter">
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            >
              <img src="/pagos-logo.png" alt="Pagos" className="h-8 w-auto" />
              <span className="text-lg font-medium text-gray-600">Hierarchy Preview</span>
            </motion.div>
          </div>
        </div>
      </header>
      ...
      <main className="bg-gray-50 min-h-[calc(100vh-120px)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <motion.div
            className="bg-gray-100 rounded-2xl shadow-lg border border-gray-200 p-6 flex flex-col sticky top-8 self-start"
              ...
              <div className="mb-6">
                ...
                {isSampleData && (
                  <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-2">
                    ...
                    <p className="text-xs text-amber-700">Demo data loaded</p>
                  </div>
                )}
                      ...
                      <div key={index} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-200">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-medium text-gray-400 w-12 shrink-0">
                            Level {index + 1}{index === 0 && <span className="text-red-400 ml-0.5">*</span>}
                          </span>
                          <div className="relative flex-1">
                            <select
                              className="w-full appearance-none px-3 py-2 pr-8 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white"
                              value={level}
                          ...
                          {index === 0 && <div className="w-7 shrink-0" />}
                        </div>
                      </div>
                    )
                  })}
                  ...
                  return (
                    <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-2xl">
                      {validation.errors.map((err, index) => (
                        <div key={index} className="text-xs text-red-700">• {err}</div>
                      ))}
                    </div>
                  )
                })()}
